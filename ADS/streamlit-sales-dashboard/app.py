...
from login import login  # Import the login function you created
...
def display_sidebar(data: pd.DataFrame) -> Tuple[List[str], List[str], List[str]]:
    ...
    st.sidebar.info('Created by Priyansh Thakur')
...
def app():
    if login():
        main()  # Only show dashboard after successful login
    else:
